...
import 'cv_template.dart'; // Usamos as classes de dados daqui
...
class PdfGeneratorService {
  ...
  Future<Uint8List> generatePdf() async {
    ...
    final theme = pw.ThemeData.withFont(base: pw.Font.ttf(font), bold: pw.Font.ttf(fontBold));

    doc.addPage(
      pw.MultiPage(
        theme: theme,
        pageFormat: PdfPageFormat.a4.copyWith(
          marginTop: options.margins.top * 0.5,
          marginBottom: options.margins.bottom * 0.5,
          marginLeft: options.margins.left * 0.5,
          marginRight: options.margins.right * 0.5,
        ),
        build: (context) {
  ...
  // --- MÉTODOS DE BUILD (Tradução 1:1 do seu CvTemplate) ---
  ...
  pw.Widget _buildModernHeader() {
    ...
    return pw.Container(
      color: accent,
      padding: const pw.EdgeInsets.symmetric(horizontal: 15, vertical: 20),
      child: pw.Column(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
      ...
      children: [
        ...
        if (exp.description?.isNotEmpty ?? false)
          pw.Padding(
              ...
              child: pw.Text(exp.description!, style: pw.TextStyle(fontSize: options.fontSize, height: 1.4, color: PdfColors.grey800))
          ),
      ]
  );
    ...
    for (final skill in data.skills)
      pw.Container(
          ...
          decoration: pw.BoxDecoration(
            color: PdfColor.fromInt(options.accentColor.value).shade(0.1),
            borderRadius: pw.BorderRadius.circular(4),
          ),
          child: pw.Text(skill.name, style: pw.TextStyle(fontSize: options.fontSize, color: PdfColors.black))
      )
  ]);
  ...
  pw.Widget _buildLanguageList() => pw.Column(
      ...
      children: data.languages.map((lang) =>
          pw.Text('• ${lang.languageName} (${lang.proficiency.displayName})', style: pw.TextStyle(fontSize: options.fontSize, height: 1.6))
      ).toList()
  );
  ...
  pw.Widget _buildContactAndOtherInfo(PersonalData p, {bool isCentered = false, bool useWhiteText = false, bool useMinimalistStyle = false}) {
    ...
    if (options.includeSocialLinks && (p.linkedinUrl?.isNotEmpty ?? false)) contactWidgets.add(pw.UrlLink(destination: p.linkedinUrl!, child: pw.Text(p.linkedinUrl!.replaceAll('https://www.', ''), style: pw.TextStyle(decoration: pw.TextDecoration.underline, color: linkColor))));
    if (options.includeSocialLinks && (p.portfolioUrl?.isNotEmpty ?? false)) contactWidgets.add(pw.UrlLink(destination: p.portfolioUrl!, child: pw.Text(p.portfolioUrl!.replaceAll('https://www.', ''), style: pw.TextStyle(decoration: pw.TextDecoration.underline, color: linkColor))));
    ...
    // --- INÍCIO DA CORREÇÃO ---
    // Criamos a lista de widgets manualmente para ter controle total
    final List<pw.Widget> children = [];

    // Adiciona os widgets de contato, inserindo o separador entre eles
    for (int i = 0; i < contactWidgets.length; i++) {
      children.add(contactWidgets[i]);
    ...
    // Adiciona um separador entre os contatos e as "outras informações" se ambos existirem
    if (contactWidgets.isNotEmpty && otherItems.isNotEmpty) {
      children.add(separator);
    }
    ...
    return pw.Wrap(
      ...
      children: children, // Usa a lista que acabamos de criar
    );
  }
}